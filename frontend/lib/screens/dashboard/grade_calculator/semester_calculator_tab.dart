...
class SemesterCalculatorTab extends StatefulWidget {
  const SemesterCalculatorTab({super.key});

  @override
  State<SemesterCalculatorTab> createState() => _SemesterCalculatorTabState();
}

class _SemesterCalculatorTabState extends State<SemesterCalculatorTab> {
  ...
  void _generateFields() {
    gpaControllers = List.generate(courseCount, (_) => TextEditingController());
    creditControllers = List.generate(courseCount, (_) => TextEditingController());
  }

  double _calculateSemesterGPA() {
    double totalGPA = 0;
  ...
  void calculateSemesterGPA() {
    semesterGPA = _calculateSemesterGPA();
    setState(() {});
  }
  ...
  Widget _buildCourseRow(int index) {
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(child: _buildField('Course ${index + 1} GPA', gpaControllers[index])),
          const SizedBox(width: 10),
          Expanded(child: _buildField('Course ${index + 1} Credits', creditControllers[index])),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    // regenerate fields whenever courseCount changes
    _generateFields();
    ...
    return Container(
      ...
      child: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          _buildCountSelector('Courses:', courseCount, (v) {
            setState(() {
              courseCount = v;
            });
          }),
          ...
          _buildSection(
            'Courses Details',
            Column(
              children: List.generate(courseCount, (i) => _buildCourseRow(i)),
            ),
            icon: Icons.scoreboard_sharp,
          ),
          const SizedBox(height: 20),
          _buildButton('Calculate Semester GPA', calculateSemesterGPA),
          const SizedBox(height: 20),
          if (semesterGPA != null)
            _buildSection(
              'Result',
